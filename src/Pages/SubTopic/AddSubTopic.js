...
export default function AddSubTopic() {
    ...
    const fetchTopics = async (subjectId) => {
        try {
            ...
            const response = await axios.get(`${baseUrl}/topics?subject=${subjectId}`, {
                headers: {
                    Authorization: localStorage.getItem("token"),
                },
            })
            ...
            if (response?.data?.status) {
                ...
                const activeTopics = response?.data?.data?.filter((topic) => topic?.status === "active") || []
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${baseUrl}/subtopics`, submitData, {
                headers: {
                    Authorization: localStorage.getItem("token"),
            ...
            if (response?.data?.status) {
                ...
                toast.error(response?.data?.message || "Failed to add subtopic")
            }
        } catch (error) {
            console.error("Error adding subtopic:", error)
            toast.error(error?.response?.data?.message || "Failed to add subtopic")
        } finally {
            setLoading(false)
        }
    }
                                        ...
                                        <div className="row">
                                            ...
                                            <div className="col-lg-6 mb-4">
                                                ...
                                                <div className="subject-select-container">
                                                    ...
                                                    >
                                                        <option value="">{!formData.subject ? "Select Subject First" : "Select Topic"}</option>
                                                        {topics.map((topic) => (
                                                    ...
                                                    <Link
                                                        to={formData.subject ? `/topics/add?subject=${formData.subject}` : "/topics/add"}
                                            ...
                                            <div className="col-lg-6 mb-4">
                                                ...
                                                <input
                                                    type="text"
                                                    className={`form-control ${errors.sub_topic ? "is-invalid" : ""}`}
                                                    id="sub_topic"
                                                    name="sub_topic"
                                                    ...
                                                    onChange={handleChange}
                                                />
                                                {errors.sub_topic && <div className="invalid-feedback">{errors.sub_topic}</div>}
                                            </div>
